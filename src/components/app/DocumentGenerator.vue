<template>
  ...
  <div class="documentGenerator-controller mb-5 flex justify-end">
    <button v-if="this.documentStatus === 'Готов' && (this.$store.state.auth.user.roles.includes('ROLE_MODERATOR') || this.$store.state.auth.user.roles.includes('ROLE_ADMIN'))" class="main-btn-yellow px-5 py-3 text-lg flex items-center gap-1" :disabled="!this.documentInfo.canChange" @click.prevent="remakeDocumentModal">
