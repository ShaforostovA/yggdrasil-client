    ...
    <div class="w-4/5 flex flex-col mb-5">
      ...
      <div class="filter shadow-md rounded-xl p-5 mb-6 border" ref="filter" :style="this.style">
        <div class="clear-filter mb-5 flex justify-end">
          <button @click.prevent="this.minValueDatePole = ''; this.maxValueDatePole = ''; this.resetKey += 1" class="main-btn px-3 py-2 flex">
        ...
        <div class="filter-item mb-5 flex gap-5">
          <div class="filter-item-inner w-1/2">
            ...
            <div class="filter-item-body">
              <SelectVue class="search-field" :selected="true" :message="'Выберите параметр сортировки'" :content="['Номер факультета', 'Наименование', 'Статус']" :key="this.resetKey"/>
            </div>
          </div>
          <div class="filter-item-inner w-1/2">
            <div class="filter-item-header mb-1">
        ...
        <div class="filter-item w-full mb-5">
          <div class="filter-item-header mb-1">
            Статус факультета:
          </div>
          <div class="filter-item-body">
            <SelectVue class="search-status" :type="'filter'" :selected="true" :message="'Выберите статус факультета'" :content="['Активный', 'Деактивированный']" :key="this.resetKey"/>
          </div>
        </div>
      </div>
      <div class="main-table-menu pb-6 relative z-10 bg-white">
        <ul class="main-table-list flex flex-row-reverse">
          <li class="main-table-item">
            <router-link :to="{name: 'facultyCreate'}" class="main-btn py-3 px-5 flex">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 mr-1">
      ...
      <div class="flex">
        <div class="">
          Всего найдено факультетов: {{ this.countFaculty }}
        </div>
        <Paginate
...
export default {
  ...
  data: () => ({
    loading: true,
    content: [],
    header: ["№ факультета", "Наименование", "Стату", "Дата создания", ""],
    noDataMessage: "По вашему запросу факультетов не найдено",
    message: '',
  ...
  methods: {
    async refresh() {
      ...
      if (this.sortField === 'Выберите параметр сортировки') {
        this.sortField = 'id';
      } else if (this.sortField === 'Номер факультета') {
        this.sortField = 'id';
      ...
      await FacultyService.getAllFacultySearch(this.parameter, this.currentPage, this.sortField, this.sortDir, this.isActive, this.minValueDatePole, this.maxValueDatePole).then(
        (response) => {
          this.content = [];
          this.countFaculty = response.data.totalItems;
          this.countPage = response.data.totalPages;
          for(const item of response.data.facultyList) {
            this.content.push([
              {
                'type': 'id',
                'text': item.id
