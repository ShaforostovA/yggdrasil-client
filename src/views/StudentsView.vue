<template>
  ...
  <div class="flex gap-x-5 flex-grow">
    ...
    <div class="w-4/5 flex flex-col mb-5">
      ...
      <div class="filter shadow-md rounded-xl p-5 mb-6 border" ref="filter" :style="this.style">
        ...
        <div class="filter-item mb-5 flex gap-5">
          <div class="filter-item-inner w-1/2">
            ...
            <div class="filter-item-body">
              <SelectVue class="search-field" :selected="true" :message="'Выберите параметр сортировки'" :content="['Номер студента', 'ФИО', 'Кафедра', 'Группа', 'Статус']" :key="this.resetKey"/>
            </div>
          </div>
          <div class="filter-item-inner w-1/2">
            <div class="filter-item-header mb-1">
        ...
        <div class="filter-item mb-5 flex gap-5">
          <div class="filter-item-inner w-1/2">
            ...
            <div class="filter-item-body">
              <SelectVue class="search-department" :type="'filter'" :selected="true" :message="'Выберите кафедру студента'" :content="this.departmentFindList" :key="this.departmentKey"/>
            </div>
          </div>
          <div class="filter-item-inner w-1/2">
            <div class="filter-item-header mb-1">
            ...
            <div class="filter-item-body">
              <SelectVue class="search-status" :type="'filter'" :selected="true" :message="'Выберите статус студента'" :content="['Учится', 'Не учится']" :key="this.resetKey"/>
            </div>
          </div>
        </div>
      </div>
      <div class="main-table-menu pb-6 bg-white relative z-10">
        <ul class="main-table-list flex flex-row-reverse">
          <li class="main-table-item">
            <router-link :to="{name: 'studentCreate'}" class="main-btn py-3 px-5 flex">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 mr-1">
      ...
      <div class="flex">
        <div class="">
          Всего найдено студентов: {{ this.countStudents }}
        </div>
        <Paginate
...
export default {
  ...
  data: () => ({
    loading: true,
    content: [],
    header: ["№ студента", "ФИО", "Кафедра", "Группа", "Статус", "Дата создания", ""],
    noDataMessage: "По вашему запросу студенты не найдены",
    message: '',
    listDepartments: [],
  ...
  methods: {
    async refresh() {
      ...
      if (this.sortField === 'Выберите параметр сортировки') {
        this.sortField = 'id';
      } else if (this.sortField === 'Номер студента') {
        this.sortField = 'id';
      } else if (this.sortField === 'ФИО') {
        this.sortField = 'lastName';
      } else if (this.sortField === 'Кафдера') {
        this.sortField = 'department';
      } else if (this.sortField === 'Группа') {
      ...
      if (this.departmentName === 'Выберите кафедру студента') {
        this.departmentName = '';
      }

      if (document.querySelector('.search-status li.selected').innerText === 'Учится') {
        this.isActive = true;
      } else if (document.querySelector('.search-status li.selected').innerText === 'Не учится') {
        this.isActive = false;
      } else {
        this.isActive = '';
      }

      // if (this.currentUser.roles[0] === 'ROLE_SUPER_ADMIN' || this.currentUser.roles[0] === 'ROLE_ADMIN') {
        await StudentService.getAllStudent(this.parameter, this.currentPage, this.sortField, this.sortDir, this.minValueDatePole, this.maxValueDatePole, this.departmentName, this.isActive).then(
          (response) => {
            this.content = [];
            this.countStudents = response.data.totalItems;
            this.countPage = response.data.totalPages;
            for(const item of response.data.studentList) {
              this.content.push([
                {
                  'type': 'id',
                  'text': item.id
                ...
                {
                  'type': 'status',
                  'text': item.isTrained ? "Учится" : "Не учится"
                },
                {
                  'type': 'date',
      ...
      //     }
      //   )
      //   await StudentService.getAllDepartmentStudent(this.parameter, this.currentPage, this.sortField, this.sortDir, this.minValueDatePole, this.maxValueDatePole, this.departmentName, this.isActive, departmentId).then(
      //     (response) => {
      //       this.content = [];
      //       this.countStudents = response.data.totalItems;
      //       this.countPage = response.data.totalPages;
