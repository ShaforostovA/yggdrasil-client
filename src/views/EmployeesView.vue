    ...
    <div class="w-4/5 flex flex-col mb-5">
      ...
      <div class="filter shadow-md rounded-xl p-5 mb-6 border" ref="filter" :style="this.style">
        <div class="clear-filter mb-5 flex justify-end">
          <button @click.prevent="this.minValueDatePole = ''; this.maxValueDatePole = ''; this.resetKey += 1; this.departmentKey += 1" class="main-btn px-3 py-2 flex">
        ...
        <div class="filter-item mb-5 flex gap-5">
          <div class="filter-item-inner w-1/2">
            ...
            <div class="filter-item-body">
              <SelectVue class="search-field" :selected="true" :message="'Выберите параметр сортировки'" :content="['Номер пользователя', 'ФИО', 'Роль', 'Кафедра', 'Статус', 'Штатность']" :key="this.resetKey"/>
            </div>
          </div>
          <div class="filter-item-inner w-1/2">
            <div class="filter-item-header mb-1">
        ...
        <div class="filter-item mb-5 flex gap-5">
          <div class="filter-item-inner w-1/2">
            ...
            <div class="filter-item-body">
              <SelectVue class="search-role" :type="'filter'" :selected="true" :message="'Выберите роль пользователя'" :content="['Преподаватель', 'Зав. кафедрой']" :key="this.resetKey"/>
            </div>
          </div>
          <div class="filter-item-inner w-1/2">
            <div class="filter-item-header mb-1">
            ...
            <div class="filter-item-body">
              <SelectVue class="search-department" :type="'filter'" :selected="true" :message="'Выберите кафедру пользователя'" :content="this.departmentFindList" :key="this.departmentKey"/>
            </div>
          </div>
        </div>
        <div class="filter-item mb-5 flex gap-5">
          <div class="filter-item-inner w-1/2">
            ...
            <div class="filter-item-body">
              <SelectVue class="search-status" :type="'filter'" :selected="true" :message="'Выберите статус пользователя'" :content="['Активный', 'Деактивированный']" :key="this.resetKey"/>
            </div>
          </div>
          <div class="filter-item-inner w-1/2">
            <div class="filter-item-header mb-1">
            ...
            <div class="filter-item-body">
              <SelectVue class="search-is-state" :type="'filter'" :selected="true" :message="'Выберите штатноть пользователя'" :content="['Штатный', 'Совместитель']" :key="this.resetKey"/>
            </div>
          </div>
        </div>
      </div>
      <div class="main-table-menu pb-6 bg-white relative z-10">
        <ul class="main-table-list flex flex-row-reverse">
          <li class="main-table-item">
            <router-link :to="{name: 'employeeCreate'}" class="main-btn py-3 px-5 flex">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 mr-1">
      ...
      <div v-if="!this.loading && this.listDepartments.length === 0" class="message px-3 py-6 text-center text-yellow-600 rounded-lg border-yellow-600 border-2 mb-6 text-lg">Невозможно создать пользователя, пока нет кафедр. <router-link v-if="this.currentUser.roles[0] === 'ROLE_SUPER_ADMIN'" to="/department/create" class="main-btn">Создать кафедру</router-link>.</div>
      <MainTable class="bg-white" v-if="!this.loading"
        :header="header" :content="content" :noDataMessage="noDataMessage"
      />
      <div class="flex">
        <div class="">
          Всего найдено сотрудников: {{ this.countEmployees }}
        </div>
        <Paginate
...
export default {
  ...
  data: () => ({
    loading: true,
    content: [],
    header: ["№ сотрудника", "ФИО", "Роль", "Кафедра", "Статус", "Штатный", "Дата создания", ""],
    noDataMessage: "По вашему запросу сотрудники не найдены",
    message: '',
    listDepartments: [],
  ...
  async mounted() {
    ...
    DepartmentService.getDepartmentsAll().then(
      (response) => {
        this.listDepartments = response.data;
        this.departmentFindList = [];
        for (let item of response.data) {
          this.departmentFindList.push(item.shortName);
        }
  ...
  methods: {
    async refresh() {
      ...
      if (this.sortField === 'Выберите параметр сортировки') {
        this.sortField = 'id';
      } else if (this.sortField === 'Номер пользователя') {
        this.sortField = 'id';
      } else if (this.sortField === 'ФИО') {
        this.sortField = 'last_name';
      } else if (this.sortField === 'Роль') {
        this.sortField = 'ur.role_id';
      } else if (this.sortField === 'Кафедра') {
        this.sortField = 'department_id';
      } else if (this.sortField === 'Статус') {
        this.sortField = 'is_active';
      } else if (this.sortField === 'Штатность') {
        this.sortField = 'is_state';
      }
      ...
      this.userRole = document.querySelector('.search-role li.selected').innerText;

      if (this.userRole === 'Выберите роль пользователя') {
        this.userRole = '';
      } else if (this.userRole === 'Преподаватель') {
        this.userRole = 'ROLE_USER';
      } else if (this.userRole === 'Зав. кафедрой') {
        this.userRole = 'ROLE_MODERATOR';
      }

      this.departmentName = document.querySelector('.search-department li.selected').innerText;

      if (this.departmentName === 'Выберите кафедру пользователя') {
        this.departmentName = '';
      }

      if (document.querySelector('.search-status li.selected').innerText === 'Активный') {
        this.isActive = true;
      } else if (document.querySelector('.search-status li.selected').innerText === 'Деактивированный') {
        this.isActive = false;
      } else {
        this.isActive = '';
      }

      if (document.querySelector('.search-is-state li.selected').innerText === 'Штатный') {
        this.isState = true;
      } else if (document.querySelector('.search-is-state li.selected').innerText === 'Совместитель') {
        this.isState = false;
      } else {
        this.isState = '';
      }

      await userService.getAllUsersFind(this.parameter, this.currentPage, this.sortField, this.sortDir, this.minValueDatePole, this.maxValueDatePole, this.departmentName, this.isState, this.userRole, this.isActive).then(
        (response) => {
          this.content = [];
          this.countEmployees = response.data.totalItems;
